...
- **Return leg (AD-2, added 2026-08-07):** `/missions-complete` (this repo) advances the session past the mission stages and lands on `/debrief` — it is what breaks the `/workspace` re-mint loop after Mission 4. The app half is docside [#64](https://github.com/manderso90/docside/pull/64): the preview banner's "Finished? Continue to the debrief" link, gated on `NEXT_PUBLIC_FIRST_LOOK_RETURN_URL`. The journey e2e now drives the real return route instead of the dev-only button.
