...
**Still to build (change-list items 5–6 + wiring):** `generateLink` session handoff + mission chrome + telemetry beacon in the app (in progress — docside PR D1); TTL reaper; metrics exclusion; one-time prod seed run; shell-side provisioning (invite → preview agent + clone via docside's service-authenticated provision route — founder-approved seam 2026-08-07) + `first_look` schema migration (lands in **docside's** `supabase/migrations/`, per `src/lib/store/supabase.ts`) + Vercel env wiring (Phase 5).
